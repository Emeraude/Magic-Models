...
exports.defineModels = {
	...
	user: function(test) {
	    test.deepEqual(db.models.User.fields, {id: {fieldName: 'id', type: 'int', key: 'primary', default: {created: null, modified: null}}, login: {fieldName: 'login', type: 'varchar', length: 32, key: 'unique', validate: [{name: 'len', fun: db.validate.len, val: [4, 32]}, {name: 'is', fun: db.validate.is, val: /^[a-z0-9\-_\.]{4,32}$/i}, {name: 'notIn', fun: db.validate.notIn, val: ['root', 'admin']}, {name: 'required', fun: db.validate.required, val: true}, {name: 'isUnique', fun: db.validate.isUnique, val: true}], default: { created: 'anonymous', modified: null}}, password: {fieldName: 'password', type: 'varchar', length: 255, validate: [{msg: 'Password not strong enough', name: 'minLen', fun: db.validate.minLen, val: 8}, {msg: 'Password not strong enough', name: 'required', fun: db.validate.required, val: true}], default: {created: null, modified: null}}, mail: {fieldName: 'mail', type: 'varchar', length: 255, validate: [{name: 'maxLen', fun: db.validate.maxLen, val: 255}], default: {created: null, modified: null}}}, 'Model `User` not defined correctly');
