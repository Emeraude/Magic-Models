...
module.exports.validate = function(orm, model, data) {
	...
	if (field.validate) {
		...
		if (orm.validate[rule]) {
			...
			if (!orm.validate[rule](data[fieldName], config.val, data, model))
			    errors.push(config.msg);
		    }
		    else {
			if (!orm.validate[rule](data[fieldName], config, data, model))
			...
			_.each(config, function(value, rule) {
			    if (rule != 'msg') {
				if (orm.validate[rule]) {
				    if (!orm.validate[rule](data[fieldName], value, data, model))
					errors.push(config.msg);
				}
				else {
